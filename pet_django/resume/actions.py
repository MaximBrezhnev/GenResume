...
def convert_text_to_dict(text: str) -> list[dict[str : str | str : list[str]]]:
    position_list = []
...
def reformat_extracted_text(text, sample_style_sheet, flowables):
    text = "#$Позиция".join(text.split("Позиция"))
...
def extend_document(document_id: uuid.UUID, data: dict[str:str]) -> str:
    folder_path = BASE_DIR / "resume" / "documents"
